...
export function registerGanttRoutes(app, pool, { auth, requireRole, tecCanProject }) {
  ...
  app.post(
    ...
    upload.single("file"),
    async (req, res) => {
      const project = (await pool.query("SELECT id FROM projects WHERE id=$1 AND organization_id=$2", [req.params.id, req.user.organizationId])).rows[0];
  ...
  app.get("/api/projects/:id/gantt-tasks", auth, async (req, res) => {
    ...
    const { rows } = await pool.query(
      "SELECT data FROM gantt_tasks WHERE project_id = $1 AND organization_id=$2 ORDER BY data->>'wbs'",
  ...
  app.post("/api/projects/:id/gantt-tasks", auth, requireRole("admin", "gerente"), async (req, res) => {
    ...
    const project = (await pool.query("SELECT id FROM projects WHERE id=$1 AND organization_id=$2", [projectId, req.user.organizationId])).rows[0];
    if (!project) return res.status(404).json({ error: "El proyecto no existe" });
    const body = req.body || {};
    ...
    if (body.parentId) {
      const parent = (await pool.query("SELECT 1 FROM gantt_tasks WHERE id=$1 AND project_id=$2 AND organization_id=$3", [body.parentId, projectId, req.user.organizationId])).rows[0];
    ...
    for (const dep of predecessors) {
      const exists = (await pool.query("SELECT 1 FROM gantt_tasks WHERE id=$1 AND project_id=$2 AND organization_id=$3", [dep.taskId, projectId, req.user.organizationId])).rows[0];
    ...
    await pool.query("INSERT INTO gantt_tasks (id, project_id, data, organization_id) VALUES ($1, $2, $3, $4)", [id, projectId, data, req.user.organizationId]);
    res.json(data);
  });
  ...
  app.patch("/api/gantt-tasks/:id", auth, requireRole("admin", "gerente"), async (req, res) => {
    const current = (await pool.query("SELECT data, project_id FROM gantt_tasks WHERE id=$1 AND organization_id=$2", [req.params.id, req.user.organizationId])).rows[0];
    ...
    if (merged.parentId) {
      const parent = (await pool.query("SELECT 1 FROM gantt_tasks WHERE id=$1 AND project_id=$2 AND organization_id=$3", [merged.parentId, current.project_id, req.user.organizationId])).rows[0];
    ...
    for (const dependency of merged.predecessors) {
      const exists = (await pool.query("SELECT 1 FROM gantt_tasks WHERE id=$1 AND project_id=$2 AND organization_id=$3", [dependency.taskId, current.project_id, req.user.organizationId])).rows[0];
      if (!exists) return res.status(400).json({ error: `La tarea predecesora ${dependency.taskId} no existe en este proyecto` });
    }
    const projectTasks = (await pool.query("SELECT id,data FROM gantt_tasks WHERE project_id=$1 AND organization_id=$2", [current.project_id, req.user.organizationId])).rows.map((row) => [row.id, row.id === req.params.id ? merged : row.data]);
    ...
    await pool.query("UPDATE gantt_tasks SET data=$2, updated_at=now() WHERE id=$1 AND organization_id=$3", [req.params.id, merged, req.user.organizationId]);
    res.json(merged);
  });
  ...
  app.delete("/api/gantt-tasks/:id", auth, requireRole("admin", "gerente"), async (req, res) => {
    const row = (await pool.query("SELECT data, project_id FROM gantt_tasks WHERE id=$1 AND organization_id=$2", [req.params.id, req.user.organizationId])).rows[0];
    if (!row) return res.status(404).json({ error: "No existe" });
    const { rows: siblings } = await pool.query("SELECT id, data FROM gantt_tasks WHERE project_id=$1 AND organization_id=$2", [row.project_id, req.user.organizationId]);
    for (const sibling of siblings) {
      ...
      if (changed) await pool.query("UPDATE gantt_tasks SET data=$2, updated_at=now() WHERE id=$1 AND organization_id=$3", [sibling.id, data, req.user.organizationId]);
    }
    await pool.query("DELETE FROM gantt_tasks WHERE id=$1 AND organization_id=$2", [req.params.id, req.user.organizationId]);
    res.status(204).end();
  });

  app.delete("/api/projects/:id/gantt-tasks", auth, requireRole("admin", "gerente"), async (req, res) => {
    await pool.query("DELETE FROM gantt_tasks WHERE project_id=$1 AND organization_id=$2", [req.params.id, req.user.organizationId]);
    res.status(204).end();
  });
}
