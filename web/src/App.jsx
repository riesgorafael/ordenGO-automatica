...
const timelineErrors = (technical, now = Date.now()) => {
  ...
  if (Number.isFinite(arrival) && Number.isFinite(end) && end >= arrival) {
    ...
    if (effectiveMinutes > onSiteMinutes) errors.push("El tiempo efectivo no puede superar el tiempo total en planta.");
...
function OrderDetail({ ger, order, onClose, onUpdate, onAdvance, onExport, onDelete, onComment, onDuplicate, onCreateTask, onEdit, me }) {
  ...
  const downloadReport = (audience) => {
    ...
    if (audience === "internal") internalOrderReportPDF(order); else clientOrderReportPDF(order);
  };
...
function OrderEditDialog({ order, clients, users, parts, onClose, onSave }) {
  ...
  const save = async () => {
    ...
    if (chronologyErrors.length) { alert(chronologyErrors.join("\n")); return; }
    setSaving(true);
