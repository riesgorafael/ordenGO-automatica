...
const EMPTY_TECHNICAL = {
  assetTag: "", manufacturer: "", model: "", serial: "", reportedAt: "", arrivalAt: "", startedAt: "", completedAt: "", downtimeMinutes: "",
  ...
  ioVerified: "", alarmsVerified: "", setpointChanges: "", workPermit: "", lotoApplied: false, ppe: "", safetyNotes: "", internalNotes: "", warranty: "", recurrence: "",
...
export default function App() {
  ...
  const exportCSV = (rows, name) => {
    const head = ["Folio", "Fecha", "Cliente", "Sitio", "Tipo", "Estado", "Horas efectivas", "Técnicos", "Horas-técnico", "Mano de obra (USD)", "Materiales (USD)", "Total (USD)"];
...
function OrderDetail({ ger, order, onClose, onUpdate, onAdvance, onExport, onDelete, onComment, onDuplicate, onCreateTask, onEdit, me }) {
  ...
  const [rate, setRate] = useState(wholeMoney(order.rate || DEFAULT_RATE));
  ...
  useEffect(() => { setRate(wholeMoney(order.rate || DEFAULT_RATE)); setMats((order.materials || []).map((material) => ({ ...material, price: wholeMoney(material.price), cost: wholeMoney(material.cost) }))); setLaborBillable(order.laborBillable); setLaborCost(wholeMoney(order.laborCost)); setSig(null); setSigBy(""); }, [order.id]);
  ...
  const savePrices = () => onUpdate(order.id, { rate: wholeMoney(rate), laborCost: wholeMoney(laborCost), materials: mats.map((m) => ({ ...m, price: wholeMoney(m.price), cost: wholeMoney(m.cost), qty: Number(m.qty) || 0 })), laborBillable });
      ...
      <div className="mobile-dialog w-full max-w-lg overflow-y-auto rounded-t-2xl bg-white sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mobile-sheet-content space-y-4 p-4 sm:p-5">
          ...
          <section><h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">Mano de obra y materiales</h4><div className="rounded-lg border border-slate-200 p-3 text-sm">
            <div className="flex items-center justify-between text-slate-600"><span>Horas efectivas</span><span className="font-medium text-slate-800">{order.laborHours || 0} h · {order.technicians || 1} téc. · {(Number(order.laborHours) || 0) * (Number(order.technicians) || 1)} h-técnico</span></div>
            {ger && <div className="mt-2 flex items-center gap-2"><span className="text-slate-600">Tarifa/h por técnico (USD):</span><input type="number" min="0" step="1" value={rate} onChange={(e) => setRate(e.target.value)} onBlur={(e) => setRate(wholeMoney(e.target.value))} className="w-24 rounded-md border border-slate-200 px-2 py-1 text-sm" /><label className="ml-auto flex items-center gap-1.5 text-xs text-slate-600"><input type="checkbox" checked={laborBillable} onChange={(e) => setLaborBillable(e.target.checked)} /> Facturable</label></div>}
          ...
          {!order.signatureUrl && order.status !== "Borrador" && (<section><h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">Firma del cliente</h4><SignaturePad key={order.id} onChange={setSig} /><input value={sigBy} onChange={(e) => setSigBy(e.target.value)} placeholder="Nombre de quien firma" className="mt-2 w-full rounded-lg border border-slate-200 px-2.5 py-2 text-sm outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20" /><div className="mt-2 rounded-lg bg-slate-50 px-3 py-2 text-xs text-slate-600">Empresa: <b>{order.client}</b></div><button disabled={!sig} onClick={() => onUpdate(order.id, { signatureUrl: sig, signedBy: sigBy, technical: { ...(order.technical || {}), signerCompany: order.client } })} className="mt-2 w-full rounded-lg bg-brand-500 px-3 py-2 text-sm font-medium text-white hover:bg-brand-400 disabled:opacity-50">Guardar firma</button></section>)}
...
function OrderEditDialog({ order, clients, users, parts, onClose, onSave }) {
  ...
  const [form, setForm] = useState(() => ({ ...order, rate: wholeMoney(order.rate || DEFAULT_RATE), laborCost: wholeMoney(order.laborCost), technical: { ...(order.technical || {}) }, materials: (order.materials || []).map(hydrateMaterial) }));
  ...
  const save = async () => {
    ...
    const timelineChanged = ["reportedAt", "arrivalAt", "startedAt", "completedAt"].some((field) => (form.technical[field] || "") !== (order.technical?.[field] || ""));
    ...
    if (timelineChanged && !timelineReasonUpdated) return;
    setSaving(true);
    const technical = { ...form.technical, signerCompany: form.client.trim() };
    if (timelineChanged && technical.startedAt) technical.workSessions = [{ start: technical.startedAt, end: technical.completedAt || null }];
    const adjustedLaborHours = timelineChanged ? round2(timelineWorkMs(technical, technical.completedAt ? new Date(technical.completedAt).getTime() : Date.now()) / 3600000) : (Number(form.laborHours) || 0);
    const patch = {
      ...
      laborHours: adjustedLaborHours, technicians: Number(form.technicians) || 1, rate: wholeMoney(form.rate || DEFAULT_RATE),
...
function NewOrder({ ger, me, clients, parts = [], onSave, onCancel }) {
  ...
  const [rate, setRate] = useState(wholeMoney(draft?.rate ?? DEFAULT_RATE)); const [laborHours, setLaborHours] = useState(draft?.laborHours || ""); const [technicians, setTechnicians] = useState(draft?.technicians || 1); const [laborBillable, setLaborBillable] = useState(draft?.laborBillable ?? true);
  ...
  const preview = orderTotals({ laborHours: automaticLaborHours, technicians, rate, laborBillable, materials });
  ...
  const save = async (status) => {
    ...
    const o = { client: client.name, site: resolvedSite, contact, tech, service, date: todayStr(), createdAt: new Date().toISOString(), equipo, sintoma, solucion, category, technical: { ...technical, signerCompany: client.name, downtimeMinutes: Number(technical.downtimeMinutes) || 0 }, location: savedLocation, photos, signatureUrl, signedBy, noSignReason: signatureUrl ? "" : noSignReason.trim(), laborHours: automaticLaborHours, technicians: Math.max(1, Math.round(Number(technicians) || 1)), rate: wholeMoney(rate) || DEFAULT_RATE, currency: "USD", laborBillable, materials: materials.map((m) => ({ ...m, qty: m.unit === "u" ? Math.max(1, Math.round(Number(m.qty) || 1)) : (Number(m.qty) || 0), price: wholeMoney(m.price), cost: wholeMoney(m.cost) })), status };
  ...
  return (
    <div className="min-h-screen bg-slate-100" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <main className="mx-auto max-w-lg space-y-4 px-3 py-4 pb-40 sm:px-4 sm:py-5 sm:pb-32">
        ...
        <ServiceTimeline technical={technical} active={activeWork} elapsedMs={elapsedWorkMs} technicians={technicians} onAction={timelineAction} onDowntime={(value) => setTechnicalField("downtimeMinutes", value)} />
        {step === 0 && (<>
        ...
        <Section title="Mano de obra">
          ...
          <div className={`mt-2 grid gap-2 ${ger ? "grid-cols-2 min-[430px]:grid-cols-3" : "grid-cols-2"}`}><L label="Horas efectivas"><input readOnly value={automaticLaborHours} className="u-input bg-slate-50" /></L><L label="Técnicos en planta"><input type="number" min="1" step="1" value={technicians} onChange={(e) => setTechnicians(e.target.value)} onBlur={(e) => setTechnicians(Math.max(1, Math.round(Number(e.target.value) || 1)))} className="u-input" /></L>{ger && <L label="Tarifa/h por técnico (USD)"><input type="number" min="0" step="1" value={rate} onChange={(e) => setRate(e.target.value)} onBlur={(e) => setRate(wholeMoney(e.target.value))} className="u-input" /></L>}</div>
        ...
        <Section title="Conformidad del cliente"><SignaturePad onChange={setSignatureUrl} /><input value={signedBy} onChange={(e) => setSignedBy(e.target.value)} placeholder="Nombre de quien firma" className="u-input mt-2" /><div className="mt-2 grid grid-cols-2 gap-2"><input value={technical.signerRole} onChange={(e) => setTechnicalField("signerRole", e.target.value)} placeholder="Cargo" className="u-input" /><div><span className="mb-1 block text-[11px] font-medium text-slate-500">Empresa</span><div className="u-input flex items-center bg-slate-50 text-slate-700" title="Se toma automáticamente del cliente seleccionado">{client?.name || "—"}</div></div></div>{!signatureUrl && (<div className="mt-2"><p className="mb-1 text-[11px] text-amber-600">Se recomienda la firma del cliente. Si no es posible, indica el motivo para poder completar igual:</p><input value={noSignReason} onChange={(e) => setNoSignReason(e.target.value)} placeholder="Motivo (ej. cliente ausente)" className="u-input" /></div>)}</Section>
        <Box className="p-4"><h3 className="mb-3 text-sm font-semibold text-slate-900">Resumen antes de enviar</h3><div className="space-y-1.5 text-sm text-slate-600"><div className="flex justify-between gap-3"><span>Cliente</span><span className="text-right font-medium text-slate-800">{client?.name || "—"}</span></div><div className="flex justify-between gap-3"><span>Servicio</span><span className="text-right font-medium text-slate-800">{service}</span></div><div className="flex justify-between gap-3"><span>Horas efectivas</span><span className="font-medium text-slate-800">{automaticLaborHours} h × {technicians || 1} técnico(s) = {round2(automaticLaborHours * (Number(technicians) || 1))} h-técnico</span></div><div className="flex justify-between gap-3"><span>Materiales</span><span className="font-medium text-slate-800">{materials.length}</span></div>{ger && <><div className="flex justify-between gap-3 border-t border-slate-100 pt-2"><span>Mano de obra</span><span>{money(preview.labor)}</span></div><div className="flex justify-between gap-3"><span>Materiales</span><span>{money(preview.mats)}</span></div><div className="flex justify-between gap-3 font-semibold text-slate-900"><span>Total</span><span>{money(preview.total)}</span></div></>}</div></Box>
        </>)}
      </main>
...
function ServiceTimeline({ technical, active, elapsedMs, technicians, onAction, onDowntime }) {
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4">
    ...
    <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4"><div className="rounded-lg bg-slate-50 px-2.5 py-2 text-xs"><span className="block text-[10px] text-slate-400">Tiempo efectivo</span><b className="text-slate-700">{compactDuration(elapsedMs)}</b></div><div className="rounded-lg bg-slate-50 px-2.5 py-2 text-xs"><span className="block text-[10px] text-slate-400">Horas-técnico</span><b className="text-slate-700">{round2((elapsedMs / 3600000) * (Number(technicians) || 1))} h</b></div>{responseMs > 0 && <div className="rounded-lg bg-slate-50 px-2.5 py-2 text-xs"><span className="block text-[10px] text-slate-400">Respuesta</span><b className="text-slate-700">{compactDuration(responseMs)}</b></div>}{onSiteMs > 0 && <div className="rounded-lg bg-slate-50 px-2.5 py-2 text-xs"><span className="block text-[10px] text-slate-400">Total en planta</span><b className="text-slate-700">{compactDuration(onSiteMs)}</b></div>}</div>
    <L label="Parada de producción (minutos, independiente del tiempo de servicio)"><input type="number" min="0" step="1" value={technical.downtimeMinutes ?? ""} onChange={(event) => onDowntime(event.target.value)} onBlur={(event) => onDowntime(Math.max(0, Math.round(Number(event.target.value) || 0)))} className="u-input mt-2" /></L>
  </section>;
}
