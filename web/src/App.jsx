...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-100 text-slate-800" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      ...
      {editingOrder && <OrderEditDialog order={orders.find((o) => o.id === editingOrder.id) || editingOrder} clients={clients} users={users} onClose={() => setEditingOrder(null)} onSave={async (patch) => { const saved = await updateOrder(editingOrder.id, patch); if (saved) { setEditingOrder(null); toast(`Orden ${editingOrder.id} actualizada`, "success"); } return saved; }} />}
...
function OrderEditDialog({ order, clients, users, onClose, onSave }) {
  const [form, setForm] = useState(() => ({ ...order, technical: { ...(order.technical || {}) }, materials: (order.materials || []).map((material) => ({ ...material })) }));
    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-slate-900/60 sm:items-center sm:p-4" onClick={onClose}>
      <div className="mobile-dialog mobile-sheet-content w-full max-w-3xl overflow-y-auto rounded-t-2xl bg-white p-4 shadow-2xl sm:rounded-2xl sm:p-5" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="space-y-5">
          ...
          <section><div className="mb-2 flex items-center justify-between"><h3 className="text-xs font-semibold uppercase tracking-wide text-slate-400">Materiales · USD</h3><button onClick={addMaterial} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs font-medium text-slate-600"><Plus className="h-3.5 w-3.5" /> Material</button></div><div className="space-y-2">{form.materials.length === 0 && <div className="rounded-lg border border-dashed border-slate-200 py-5 text-center text-xs text-slate-400">Sin materiales.</div>}{form.materials.map((material, index) => <div key={index} className="rounded-lg border border-slate-200 p-2.5"><div className="grid grid-cols-2 gap-2 sm:grid-cols-[minmax(0,1fr)_5rem_7rem_7rem_auto]"><input value={material.name || ""} onChange={(event) => setMaterial(index, { name: event.target.value })} placeholder="Descripción" className="u-input col-span-2 sm:col-span-1" /><input type="number" min="0" step="0.01" value={material.qty ?? 1} onChange={(event) => setMaterial(index, { qty: event.target.value })} placeholder="Cant." className="u-input" /><input type="number" min="0" step="0.01" value={material.price ?? 0} onChange={(event) => setMaterial(index, { price: event.target.value })} placeholder="Venta USD" className="u-input" /><input type="number" min="0" step="0.01" value={material.cost ?? 0} onChange={(event) => setMaterial(index, { cost: event.target.value })} placeholder="Costo USD" className="u-input" /><button onClick={() => removeMaterial(index)} aria-label="Quitar material" className="grid h-10 w-10 place-items-center rounded-lg text-slate-400 hover:bg-rose-50 hover:text-rose-500"><Trash2 className="h-4 w-4" /></button></div><label className="mt-2 inline-flex items-center gap-2 text-[11px] text-slate-500"><input type="checkbox" checked={material.billable !== false} onChange={(event) => setMaterial(index, { billable: event.target.checked })} /> Facturable</label></div>)}</div></section>
        </div>
