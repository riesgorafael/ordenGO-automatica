...
function OrderEditDialog({ order, clients, users, parts, onClose, onSave }) {
  ...
  const save = async () => {
    ...
    const patch = {
      ...
      laborHours: Number(form.laborHours) || 0, technicians: Number(form.technicians) || 1, rate: wholeMoney(form.rate || DEFAULT_RATE),
    ...
    <div className="min-h-screen bg-slate-100" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <main className="mx-auto max-w-lg space-y-4 px-3 py-4 pb-40 sm:px-4 sm:py-5 sm:pb-32">
        ...
        <Section title="Documentación del trabajo">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2"><L label="Aviso recibido"><input type="datetime-local" value={technical.reportedAt} onChange={(e) => setTechnicalField("reportedAt", e.target.value)} className="u-input" /></L><L label="Llegada al sitio"><input type="datetime-local" value={technical.arrivalAt} onChange={(e) => setTechnicalField("arrivalAt", e.target.value)} className="u-input" /></L><L label="Inicio del trabajo"><input type="datetime-local" value={technical.startedAt} onChange={(e) => setTechnicalField("startedAt", e.target.value)} className="u-input" /></L><L label="Parada (minutos)"><input type="number" min="0" step="1" value={technical.downtimeMinutes} onChange={(e) => setTechnicalField("downtimeMinutes", e.target.value)} onBlur={(e) => setTechnicalField("downtimeMinutes", Math.max(0, Math.round(Number(e.target.value) || 0)))} className="u-input" /></L></div>
        ...
        <Section title="Mano de obra">
          ...
          <div className={`mt-2 grid gap-2 ${ger ? "grid-cols-2 min-[430px]:grid-cols-3" : "grid-cols-2"}`}><L label="Horas en planta"><input type="number" min="0" step="0.5" value={laborHours} onChange={(e) => setLaborHours(e.target.value)} placeholder="0" className="u-input" /></L><L label="Técnicos en planta"><input type="number" min="1" step="1" value={technicians} onChange={(e) => setTechnicians(e.target.value)} onBlur={(e) => setTechnicians(Math.max(1, Math.round(Number(e.target.value) || 1)))} className="u-input" /></L>{ger && <L label="Tarifa/h por técnico (USD)"><input type="number" min="0" step="1" value={rate} onChange={(e) => setRate(e.target.value)} onBlur={(e) => setRate(wholeMoney(e.target.value))} className="u-input" /></L>}</div>
