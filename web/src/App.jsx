...
function OrderEditDialog({ order, clients, users, parts, onClose, onSave }) {
  const hydrateMaterial = (material) => {
    ...
    return { ...material, partId: part.id, name: part.name, unit: part.unit || material.unit, price: Number(material.price) > 0 ? material.price : (part.price ?? 0), cost: Number(material.cost) > 0 ? material.cost : (part.cost ?? 0), partNumber: material.partNumber || part.partNumber || "", brand: material.brand || part.brand || "", model: material.model || part.model || "", supplier: material.supplier || part.supplier || "" };
  };
  ...
  const selectInventoryMaterial = (index, name) => {
    ...
    setMaterial(index, { name: part.name, partId: part.id, unit: part.unit, price: Number(part.price) || 0, cost: Number(part.cost) || 0, partNumber: part.partNumber || "", brand: part.brand || "", model: part.model || "", supplier: part.supplier || "" });
  };
  ...
  const save = async () => {
    ...
    const patch = {
      ...
      materials: form.materials.map((material) => ({ ...material, name: material.name?.trim() || "", qty: Number(material.qty) || 0, price: Number(material.price) || 0, cost: Number(material.cost) || 0, billable: material.billable !== false })),
    };
