...
const EMPTY_TECHNICAL = {
  assetTag: "", manufacturer: "", model: "", serial: "", reportedAt: "", arrivalAt: "", startedAt: "", completedAt: "", downtimeMinutes: "", billableWaitMinutes: "",
...
function OrderDetail({ ger, order, onClose, onUpdate, onAdvance, onExport, onDelete, onComment, onDuplicate, onCreateTask, onEdit, me }) {
  ...
  useEffect(() => { setRate(normalizedRate(order.rate)); setMats((order.materials || []).map((material) => ({ ...material, price: wholeMoney(material.price), cost: wholeMoney(material.cost) }))); setLaborBillable(order.laborBillable); setLaborCost(wholeMoney(order.laborCost)); setSig(null); setSigBy(""); }, [order.id]);
      ...
      <div className="mobile-dialog w-full max-w-lg overflow-y-auto rounded-t-2xl bg-white sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mobile-sheet-content space-y-4 p-4 sm:p-5">
          ...
          <section><h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">Mano de obra y materiales</h4><div className="rounded-lg border border-slate-200 p-3 text-sm">
            <div className="flex items-center justify-between text-slate-600"><span>Tiempo efectivo</span><span className="font-medium text-slate-800">{compactDuration((Number(order.laborHours) || 0) * 3600000)}</span></div><div className="mt-1 flex items-center justify-between text-slate-600"><span>Horas facturables</span><span className="font-medium text-slate-800">{t.billedHours} h · {order.technicians || 1} téc. · {round2(t.billedHours * (Number(order.technicians) || 1))} h-técnico</span></div>{Number(order.technical?.billableWaitMinutes) > 0 && <div className="mt-1 flex items-center justify-between text-xs text-slate-500"><span>Espera facturable</span><span>{order.technical.billableWaitMinutes} min</span></div>}
...
function OrderEditDialog({ order, clients, users, parts, onClose, onSave }) {
  ...
  const save = async () => {
    ...
    const timelineChanged = timeFieldsChanged || ["billableWaitMinutes", "downtimeMinutes"].some((field) => (form.technical[field] || "") !== (order.technical?.[field] || ""));
  ...
  const timelineChanged = ["reportedAt", "arrivalAt", "startedAt", "completedAt", "billableWaitMinutes", "downtimeMinutes"].some((field) => (form.technical[field] || "") !== (order.technical?.[field] || ""));
  const timelineReasonUpdated = !!form.technical.timelineAdjustmentReason?.trim() && form.technical.timelineAdjustmentReason.trim() !== (order.technical?.timelineAdjustmentReason || "").trim();
  return (
...
function NewOrder({ ger, me, clients, parts = [], onSave, onCancel }) {
  ...
  const save = async (status) => {
    ...
    const o = { client: client.name, site: resolvedSite, contact, tech, service, date: todayStr(), createdAt: new Date().toISOString(), equipo, sintoma, solucion, category, technical: { ...technical, signerCompany: client.name, downtimeMinutes: Number(technical.downtimeMinutes) || 0, billableWaitMinutes: Number(technical.billableWaitMinutes) || 0 }, location: savedLocation, photos, signatureUrl, signedBy, noSignReason: signatureUrl ? "" : noSignReason.trim(), laborHours: automaticLaborHours, billableHours: projectedBillableHours, technicians: Math.max(1, Math.round(Number(technicians) || 1)), rate: normalizedRate(rate), currency: "USD", laborBillable, materials: materials.map((m) => ({ ...m, qty: m.unit === "u" ? Math.max(1, Math.round(Number(m.qty) || 1)) : (Number(m.qty) || 0), price: wholeMoney(m.price), cost: wholeMoney(m.cost) })), status };
  ...
  return (
    <div className="min-h-screen bg-slate-100" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <main className="mx-auto max-w-lg space-y-4 px-3 py-4 pb-40 sm:px-4 sm:py-5 sm:pb-32">
        ...
        <ServiceTimeline technical={technical} active={activeWork} elapsedMs={elapsedWorkMs} billableHours={projectedBillableHours} minimumApplied={minimumBillingApplied} technicians={technicians} onAction={timelineAction} onDowntime={(value) => setTechnicalField("downtimeMinutes", value)} onBillableWait={(value) => setTechnicalField("billableWaitMinutes", value)} />
        {step === 0 && (<>
        ...
        <Section title="Mano de obra">
          ...
          <div className={`mt-2 grid gap-2 ${ger ? "grid-cols-2 min-[430px]:grid-cols-4" : "grid-cols-2 min-[430px]:grid-cols-3"}`}><L label="Tiempo efectivo"><div className="u-input flex items-center bg-slate-50 font-medium text-slate-700">{compactDuration(elapsedWorkMs)}</div></L><L label="Horas facturables"><div className="u-input flex items-center bg-brand-50 font-semibold text-brand-700">{projectedBillableHours} h</div></L><L label="Técnicos en planta"><input type="number" min="1" step="1" value={technicians} onChange={(e) => setTechnicians(e.target.value)} onBlur={(e) => setTechnicians(Math.max(1, Math.round(Number(e.target.value) || 1)))} className="u-input" /></L>{ger && <L label="Tarifa/h por técnico (USD)"><input type="number" min="0" step="1" value={rate} onChange={(e) => setRate(e.target.value)} onBlur={(e) => setRate(normalizedRate(e.target.value))} className="u-input" /></L>}</div>
          {minimumBillingApplied && <p className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700">Mínimo aplicado: permanencia menor a 1 hora → se facturan 2 horas por técnico.</p>}
        ...
        <Box className="p-4"><h3 className="mb-3 text-sm font-semibold text-slate-900">Resumen antes de enviar</h3><div className="space-y-1.5 text-sm text-slate-600"><div className="flex justify-between gap-3"><span>Cliente</span><span className="text-right font-medium text-slate-800">{client?.name || "—"}</span></div><div className="flex justify-between gap-3"><span>Servicio</span><span className="text-right font-medium text-slate-800">{service}</span></div><div className="flex justify-between gap-3"><span>Tiempo efectivo</span><span className="font-medium text-slate-800">{compactDuration(elapsedWorkMs)}</span></div><div className="flex justify-between gap-3"><span>Horas facturables</span><span className="font-medium text-slate-800">{projectedBillableHours} h × {technicians || 1} técnico(s) = {round2(projectedBillableHours * (Number(technicians) || 1))} h-técnico</span></div><div className="flex justify-between gap-3"><span>Materiales</span><span className="font-medium text-slate-800">{materials.length}</span></div>{ger && <><div className="flex justify-between gap-3 border-t border-slate-100 pt-2"><span>Mano de obra</span><span>{money(preview.labor)}</span></div><div className="flex justify-between gap-3"><span>Materiales</span><span>{money(preview.mats)}</span></div><div className="flex justify-between gap-3 font-semibold text-slate-900"><span>Total</span><span>{money(preview.total)}</span></div></>}</div></Box>
        </>)}
      </main>
...
function ServiceTimeline({ technical, active, elapsedMs, billableHours, minimumApplied, technicians, onAction, onDowntime, onBillableWait }) {
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4">
    ...
    <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4"><div className="rounded-lg bg-slate-50 px-2.5 py-2 text-xs"><span className="block text-[10px] text-slate-400">Tiempo efectivo</span><b className="text-slate-700">{compactDuration(elapsedMs)}</b></div><div className={`rounded-lg px-2.5 py-2 text-xs ${minimumApplied ? "bg-amber-50" : "bg-brand-50"}`}><span className="block text-[10px] text-slate-400">Horas facturables</span><b className={minimumApplied ? "text-amber-700" : "text-brand-700"}>{billableHours} h{minimumApplied ? " · mínimo" : ""}</b></div>{responseMs > 0 && <div className="rounded-lg bg-slate-50 px-2.5 py-2 text-xs"><span className="block text-[10px] text-slate-400">Respuesta</span><b className="text-slate-700">{compactDuration(responseMs)}</b></div>}{onSiteMs > 0 && <div className="rounded-lg bg-slate-50 px-2.5 py-2 text-xs"><span className="block text-[10px] text-slate-400">Total en planta</span><b className="text-slate-700">{compactDuration(onSiteMs)}</b></div>}</div>
    <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2"><L label="Espera facturable imputable al cliente (minutos)"><input type="number" min="0" step="1" value={technical.billableWaitMinutes ?? ""} onChange={(event) => onBillableWait(event.target.value)} onBlur={(event) => onBillableWait(Math.max(0, Math.round(Number(event.target.value) || 0)))} className="u-input" /></L><L label="Parada de producción (minutos, no se factura)"><input type="number" min="0" step="1" value={technical.downtimeMinutes ?? ""} onChange={(event) => onDowntime(event.target.value)} onBlur={(event) => onDowntime(Math.max(0, Math.round(Number(event.target.value) || 0)))} className="u-input" /></L></div>
  </section>;
}
