...
function NewOrder({ ger, me, clients, parts = [], onSave, onCancel }) {
  ...
  const canComplete = canSave && chronologyErrors.length === 0 && (!!signatureUrl || !!noSignReason.trim());
  ...
  const save = async (status) => {
    ...
    const o = { client: client.name, site: resolvedSite, contact, tech, quoteNumber: quoteNumber.trim(), customerPO: customerPO.trim(), service, date: todayStr(), createdAt: new Date().toISOString(), equipo, sintoma, solucion, category, technical: { ...technical, signerCompany: client.name, downtimeMinutes: Number(technical.downtimeMinutes) || 0, billableWaitMinutes: Number(technical.billableWaitMinutes) || 0 }, location: savedLocation, photos, signatureUrl, signedAt: signatureUrl ? new Date().toISOString() : null, signedBy, noSignReason: signatureUrl ? "" : noSignReason.trim(), laborHours: automaticLaborHours, billableHours: projectedBillableHours, technicians: Math.max(1, Math.round(Number(technicians) || 1)), rate: normalizedRate(rate), currency: "USD", laborBillable, materials: materials.map((m) => ({ ...m, qty: m.unit === "u" ? Math.max(1, Math.round(Number(m.qty) || 1)) : (Number(m.qty) || 0), price: wholeMoney(m.price), cost: wholeMoney(m.cost) })), status };
  ...
  return (
    <div className="min-h-screen bg-slate-100" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <main className="mx-auto max-w-lg space-y-4 px-3 py-4 pb-40 sm:px-4 sm:py-5 sm:pb-32">
        ...
        <ServiceTimeline technical={technical} active={activeWork} elapsedMs={elapsedWorkMs} billableHours={projectedBillableHours} minimumApplied={minimumBillingApplied} technicians={technicians} errors={chronologyErrors} onAction={timelineAction} onDowntime={(value) => setTechnicalField("downtimeMinutes", value)} onBillableWait={(value) => setTechnicalField("billableWaitMinutes", value)} onBillableWaitReason={(value) => setTechnicalField("billableWaitReason", value)} />
        {step === 0 && (<>
...
function ServiceTimeline({ technical, active, elapsedMs, billableHours, minimumApplied, technicians, errors = [], onAction, onDowntime, onBillableWait, onBillableWaitReason }) {
  ...
  return <section className="rounded-xl border border-slate-200 bg-white p-4">
    ...
    <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2"><L label="Espera por condiciones del sitio (minutos)"><input type="number" min="0" step="1" value={technical.billableWaitMinutes ?? ""} onChange={(event) => onBillableWait(event.target.value)} onBlur={(event) => onBillableWait(Math.max(0, Math.round(Number(event.target.value) || 0)))} className="u-input" /></L><L label="Parada productiva informada (minutos, independiente de la visita)"><input type="number" min="0" step="1" value={technical.downtimeMinutes ?? ""} onChange={(event) => onDowntime(event.target.value)} onBlur={(event) => onDowntime(Math.max(0, Math.round(Number(event.target.value) || 0)))} className="u-input" /></L></div>
