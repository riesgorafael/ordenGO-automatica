...
export default function App() {
  ...
  const exportCSV = (rows, name) => {
    const head = ["Folio", "Fecha", "Cliente", "Sitio", "Tipo", "Estado", "Horas", "Mano de obra (USD)", "Materiales (USD)", "Total (USD)"];
...
function MonthlyReport({ orders }) {
  ...
  const exportCSV = () => {
    const head = ["Cliente", "Órdenes", "Horas", "Mano de obra (USD)", "Materiales (USD)", "Total (USD)", "Facturado (USD)", "Por facturar (USD)"];
      ...
      <div className="mobile-dialog w-full max-w-lg overflow-y-auto rounded-t-2xl bg-white sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mobile-sheet-content space-y-4 p-4 sm:p-5">
          ...
          <section><h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">Mano de obra y materiales</h4><div className="rounded-lg border border-slate-200 p-3 text-sm">
            ...
            {ger && <div className="mt-2 flex items-center gap-2"><span className="text-slate-600">Tarifa/h (USD):</span><input type="number" value={rate} onChange={(e) => setRate(e.target.value)} className="w-24 rounded-md border border-slate-200 px-2 py-1 text-sm" /><label className="ml-auto flex items-center gap-1.5 text-xs text-slate-600"><input type="checkbox" checked={laborBillable} onChange={(e) => setLaborBillable(e.target.checked)} /> Facturable</label></div>}
    ...
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-slate-900/60 sm:items-center sm:p-4" onClick={onClose}>
      <div className="mobile-dialog mobile-sheet-content w-full max-w-3xl overflow-y-auto rounded-t-2xl bg-white p-4 shadow-2xl sm:rounded-2xl sm:p-5" onClick={(event) => event.stopPropagation()}>
        ...
        <div className="space-y-5">
          ...
          <section><h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-slate-400">Mano de obra · USD</h3><div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><L label="Horas"><input type="number" min="0" step="0.25" value={form.laborHours ?? ""} onChange={(event) => set({ laborHours: event.target.value })} className="u-input" /></L><L label="Técnicos"><input type="number" min="1" value={form.technicians ?? 1} onChange={(event) => set({ technicians: event.target.value })} className="u-input" /></L><L label="Tarifa/h (USD)"><input type="number" min="0" step="0.01" value={form.rate ?? DEFAULT_RATE} onChange={(event) => set({ rate: event.target.value })} className="u-input" /></L><L label="Costo interno/h (USD)"><input type="number" min="0" step="0.01" value={form.laborCost ?? 0} onChange={(event) => set({ laborCost: event.target.value })} className="u-input" /></L></div><label className="mt-2 inline-flex items-center gap-2 text-xs text-slate-600"><input type="checkbox" checked={form.laborBillable !== false} onChange={(event) => set({ laborBillable: event.target.checked })} /> Mano de obra facturable</label></section>
