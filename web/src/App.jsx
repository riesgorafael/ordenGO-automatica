...
import { clientOrderReportPDF, internalOrderReportPDF, monthlyReportPDF } from "./pdf";
...
export default function App() {
  ...
  const exportCSV = (rows, name) => {
    const head = ["Folio", "Fecha", "Cliente", "Sitio", "Tipo", "Estado", "Horas en planta", "Técnicos", "Horas-técnico", "Mano de obra (USD)", "Materiales (USD)", "Total (USD)"];
    const lines = rows.map((o) => { const t = orderTotals(o); const technicianHours = (Number(o.laborHours) || 0) * (Number(o.technicians) || 1); return [o.id, o.date, o.client, o.site, o.service, o.status, o.laborHours, o.technicians || 1, technicianHours, t.labor.toFixed(2), t.mats.toFixed(2), t.total.toFixed(2)].map((v) => `"${String(v ?? "").replace(/"/g, '""')}"`).join(","); });
      ...
      <div className="mobile-dialog w-full max-w-lg overflow-y-auto rounded-t-2xl bg-white sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mobile-sheet-content space-y-4 p-4 sm:p-5">
          ...
          <section><h4 className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-400">Mano de obra y materiales</h4><div className="rounded-lg border border-slate-200 p-3 text-sm">
            <div className="flex items-center justify-between text-slate-600"><span>Horas en planta</span><span className="font-medium text-slate-800">{order.laborHours || 0} h · {order.technicians || 1} téc. · {(Number(order.laborHours) || 0) * (Number(order.technicians) || 1)} h-técnico</span></div>
            {ger && <div className="mt-2 flex items-center gap-2"><span className="text-slate-600">Tarifa/h por técnico (USD):</span><input type="number" value={rate} onChange={(e) => setRate(e.target.value)} className="w-24 rounded-md border border-slate-200 px-2 py-1 text-sm" /><label className="ml-auto flex items-center gap-1.5 text-xs text-slate-600"><input type="checkbox" checked={laborBillable} onChange={(e) => setLaborBillable(e.target.checked)} /> Facturable</label></div>}
            {ger && laborBillable && <p className="mt-1 text-[11px] text-slate-500">{order.laborHours || 0} h × {order.technicians || 1} técnico(s) × {money(rate)} = <b className="text-slate-700">{money(t.labor)}</b></p>}
            {ger && <div className="mt-1 flex items-center gap-2"><span className="text-slate-500 text-xs">Costo/h interno (USD):</span><input type="number" value={laborCost} onChange={(e) => setLaborCost(e.target.value)} className="w-24 rounded-md border border-slate-200 px-2 py-1 text-xs" /></div>}
            {mats.length > 0 && <ul className="mt-2 space-y-1.5 border-t border-slate-100 pt-2">{mats.map((m, i) => (<li key={i} className="text-sm"><div className="flex min-w-0 items-start justify-between gap-2"><span className="min-w-0 break-words text-slate-700">{m.qty}× {m.name || "—"}</span>{ger && <span className="shrink-0 text-xs text-slate-500">{money((m.qty || 0) * (m.price || 0))}</span>}</div>{ger && <div className="mt-1 grid grid-cols-2 gap-2 sm:flex sm:items-center"><label className="text-xs text-slate-500">P. unit. USD:<input type="number" value={m.price} onChange={(e) => setMats((x) => x.map((y, j) => j === i ? { ...y, price: e.target.value } : y))} className="mt-1 w-full rounded-md border border-slate-200 px-2 py-1 text-xs sm:ml-1 sm:mt-0 sm:w-24" /></label><label className="text-xs text-slate-500">Costo USD:<input type="number" value={m.cost ?? ""} onChange={(e) => setMats((x) => x.map((y, j) => j === i ? { ...y, cost: e.target.value } : y))} className="mt-1 w-full rounded-md border border-slate-200 px-2 py-1 text-xs sm:ml-1 sm:mt-0 sm:w-20" /></label><label className="col-span-2 flex items-center gap-1 text-[11px] text-slate-500 sm:ml-auto"><input type="checkbox" checked={m.billable} onChange={(e) => setMats((x) => x.map((y, j) => j === i ? { ...y, billable: e.target.checked } : y))} /> Facturable</label></div>}</li>))}</ul>}
          </div></section>
          ...
          <section className="flex flex-wrap gap-2 pt-1">
            ...
            <button onClick={() => clientOrderReportPDF(order)} className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50"><FileText className="h-4 w-4" /> Reporte para cliente</button>
            {ger && <button onClick={() => internalOrderReportPDF(order)} className="inline-flex items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-medium text-amber-700 hover:bg-amber-100"><FileText className="h-4 w-4" /> Informe interno</button>}
...
function OrderEditDialog({ order, clients, users, parts, onClose, onSave }) {
  const hydrateMaterial = (material) => {
    ...
    return { ...material, partId: part.id, name: part.name, unit: part.unit || material.unit, qty: quantity, price: Number(material.price) > 0 ? material.price : (part.price ?? 0), cost: Number(material.cost) > 0 ? material.cost : (part.cost ?? 0), partNumber: material.partNumber || part.partNumber || "", brand: material.brand || part.brand || "", model: material.model || part.model || "", supplier: material.supplier || part.supplier || "" };
  };
  const [form, setForm] = useState(() => ({ ...order, technical: { ...(order.technical || {}) }, materials: (order.materials || []).map(hydrateMaterial) }));
  const [saving, setSaving] = useState(false);
  const set = (patch) => setForm((current) => ({ ...current, ...patch }));
  const setTechnical = (field, value) => setForm((current) => ({ ...current, technical: { ...current.technical, [field]: value } }));
  ...
  const selectInventoryMaterial = (index, name) => {
    ...
    setMaterial(index, { name: part.name, partId: part.id, unit: part.unit, qty: part.unit === "u" ? Math.max(1, Math.round(Number(form.materials[index]?.qty) || 1)) : (form.materials[index]?.qty || 1), price: Number(part.price) || 0, cost: Number(part.cost) || 0, partNumber: part.partNumber || "", brand: part.brand || "", model: part.model || "", supplier: part.supplier || "" });
  };
  ...
  const save = async () => {
    ...
    setSaving(true);
    const patch = {
      client: form.client.trim(), site: form.site.trim(), contact: form.contact?.trim() || "", tech: form.tech?.trim() || "",
      service: form.service, date: form.date, status: form.status, equipo: form.equipo?.trim() || "", sintoma: form.sintoma?.trim() || "",
      solucion: form.solucion?.trim() || "", category: form.category?.trim() || "", technical: form.technical, currency: "USD",
      laborHours: Number(form.laborHours) || 0, technicians: Number(form.technicians) || 1, rate: Number(form.rate) || DEFAULT_RATE,
      laborCost: Number(form.laborCost) || 0, laborBillable: form.laborBillable !== false,
      materials: form.materials.map((material) => ({ ...material, name: material.name?.trim() || "", qty: material.unit === "u" ? Math.max(1, Math.round(Number(material.qty) || 1)) : (Number(material.qty) || 0), price: Number(material.price) || 0, cost: Number(material.cost) || 0, billable: material.billable !== false })),
    };
...
function NewOrder({ ger, me, clients, parts = [], onSave, onCancel }) {
  ...
  const [technical, setTechnical] = useState(draft?.technical || { assetTag: "", manufacturer: "", model: "", serial: "", reportedAt: "", arrivalAt: "", startedAt: "", completedAt: "", downtimeMinutes: "", diagnosis: "", rootCause: "", measurementsBefore: "", measurementsAfter: "", testsPerformed: "", testResult: "", finalCondition: "Operativo", recommendations: "", pendingActions: "", followUpDate: "", signerRole: "", signerCompany: "", deviceType: "", firmware: "", programVersion: "", backupRef: "", ioVerified: "", alarmsVerified: "", setpointChanges: "", workPermit: "", lotoApplied: false, ppe: "", safetyNotes: "", internalNotes: "", warranty: "", recurrence: "" });
  ...
  const preview = orderTotals({ laborHours, technicians, rate, laborBillable, materials });
  ...
  const steps = ["Activo", "Diagnóstico", "Intervención", "Cierre"];
  const stepReady = step === 0 ? !!client?.name && !!siteLabel.trim() && !!(equipo || technical.assetTag) : step === 1 ? !!(sintoma || technical.diagnosis || photos.length) : step === 2 ? !!(solucion || laborHours || materials.length) : canComplete;
  useEffect(() => {
    const timer = setTimeout(() => { saveOrderDraft(me.id, { step, clientMode, clientId, newClient, siteLabel, contact, tech, service, equipo, sintoma, solucion, category, technical, rate, laborHours, technicians, laborBillable, materials, location }); setDraftSaved(true); }, 500);
    setDraftSaved(false); return () => clearTimeout(timer);
  }, [me.id, step, clientMode, clientId, newClient, siteLabel, contact, tech, service, equipo, sintoma, solucion, category, technical, rate, laborHours, technicians, laborBillable, materials, location]);
  const save = async (status) => {
    setSaving(true);
    const resolvedSite = siteLabel.trim() || client.site || "";
    const savedLocation = location ? { ...location, label: resolvedSite } : null;
    const o = { client: client.name, site: resolvedSite, contact, tech, service, date: todayStr(), createdAt: new Date().toISOString(), equipo, sintoma, solucion, category, technical: { ...technical, downtimeMinutes: Number(technical.downtimeMinutes) || 0 }, location: savedLocation, photos, signatureUrl, signedBy, noSignReason: signatureUrl ? "" : noSignReason.trim(), laborHours: Number(laborHours) || 0, technicians: Number(technicians) || 1, rate: Number(rate) || DEFAULT_RATE, currency: "USD", laborBillable, materials: materials.map((m) => ({ ...m, qty: Number(m.qty) || 0, price: Number(m.price) || 0, cost: Number(m.cost) || 0 })), status };
    ...
    <div className="min-h-screen bg-slate-100" style={{ fontFamily: "ui-sans-serif, system-ui, sans-serif" }}>
      ...
      <main className="mx-auto max-w-lg space-y-4 px-3 py-4 pb-40 sm:px-4 sm:py-5 sm:pb-32">
        ...
        <Section title="Documentación del trabajo">
          ...
          <textarea value={technical.diagnosis} onChange={(e) => setTechnicalField("diagnosis", e.target.value)} rows={3} placeholder="Diagnóstico técnico / condición encontrada" className="u-input mt-2 resize-none" />
          <textarea value={technical.rootCause} onChange={(e) => setTechnicalField("rootCause", e.target.value)} rows={2} placeholder="Causa raíz probable o confirmada" className="u-input mt-2 resize-none" />
          <div className="mt-3 grid grid-cols-2 gap-2"><L label="Aviso recibido"><input type="datetime-local" value={technical.reportedAt} onChange={(e) => setTechnicalField("reportedAt", e.target.value)} className="u-input" /></L><L label="Llegada al sitio"><input type="datetime-local" value={technical.arrivalAt} onChange={(e) => setTechnicalField("arrivalAt", e.target.value)} className="u-input" /></L><L label="Inicio del trabajo"><input type="datetime-local" value={technical.startedAt} onChange={(e) => setTechnicalField("startedAt", e.target.value)} className="u-input" /></L><L label="Parada (minutos)"><input type="number" value={technical.downtimeMinutes} onChange={(e) => setTechnicalField("downtimeMinutes", e.target.value)} className="u-input" /></L></div>
          <div className="mt-3 rounded-lg border border-amber-200 bg-amber-50/50 p-3"><h4 className="text-xs font-semibold text-amber-800">Seguridad del trabajo</h4><div className="mt-2 grid grid-cols-2 gap-2"><L label="Permiso / autorización"><input value={technical.workPermit} onChange={(e) => setTechnicalField("workPermit", e.target.value)} className="u-input bg-white" /></L><L label="EPP utilizado"><input value={technical.ppe} onChange={(e) => setTechnicalField("ppe", e.target.value)} className="u-input bg-white" /></L></div><label className="mt-2 flex items-center gap-2 text-xs font-medium text-amber-800"><input type="checkbox" checked={technical.lotoApplied} onChange={(e) => setTechnicalField("lotoApplied", e.target.checked)} /> Bloqueo y etiquetado (LOTO) aplicado</label><textarea value={technical.safetyNotes} onChange={(e) => setTechnicalField("safetyNotes", e.target.value)} rows={2} placeholder="Riesgos, condiciones o medidas adicionales" className="u-input mt-2 resize-none bg-white" /></div>
        </Section>
        )}
        {step === 2 && (<>
        <Section title="Intervención realizada">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2"><L label="Dispositivo"><input value={technical.deviceType} onChange={(e) => setTechnicalField("deviceType", e.target.value)} placeholder="PLC, HMI, VFD…" className="u-input" /></L><L label="Firmware"><input value={technical.firmware} onChange={(e) => setTechnicalField("firmware", e.target.value)} className="u-input" /></L><L label="Versión de programa"><input value={technical.programVersion} onChange={(e) => setTechnicalField("programVersion", e.target.value)} className="u-input" /></L><L label="Referencia de respaldo"><input value={technical.backupRef} onChange={(e) => setTechnicalField("backupRef", e.target.value)} className="u-input" /></L></div>
          <textarea value={technical.ioVerified} onChange={(e) => setTechnicalField("ioVerified", e.target.value)} rows={2} placeholder="Entradas, salidas y señales verificadas" className="u-input mt-2 resize-none" />
          <textarea value={technical.alarmsVerified} onChange={(e) => setTechnicalField("alarmsVerified", e.target.value)} rows={2} placeholder="Alarmas e interlocks verificados" className="u-input mt-2 resize-none" />
          <textarea value={technical.setpointChanges} onChange={(e) => setTechnicalField("setpointChanges", e.target.value)} rows={2} placeholder="Setpoints o parámetros modificados: valor anterior → valor nuevo" className="u-input mt-2 resize-none" />
        </Section>
        <Section title="Mano de obra">
          ...
          <div className={`mt-2 grid gap-2 ${ger ? "grid-cols-2 min-[430px]:grid-cols-3" : "grid-cols-2"}`}><L label="Horas en planta"><input type="number" value={laborHours} onChange={(e) => setLaborHours(e.target.value)} placeholder="0" className="u-input" /></L><L label="Técnicos en planta"><input type="number" value={technicians} onChange={(e) => setTechnicians(e.target.value)} className="u-input" /></L>{ger && <L label="Tarifa/h por técnico (USD)"><input type="number" value={rate} onChange={(e) => setRate(e.target.value)} className="u-input" /></L>}</div>
        ...
        {step === 3 && (<>
        <Section title="Verificación y estado final"><div className="grid grid-cols-2 gap-2"><L label="Finalización"><input type="datetime-local" value={technical.completedAt} onChange={(e) => setTechnicalField("completedAt", e.target.value)} className="u-input" /></L><L label="Estado del activo"><select value={technical.finalCondition} onChange={(e) => setTechnicalField("finalCondition", e.target.value)} className="u-input"><option>Operativo</option><option>Operativo con restricciones</option><option>Fuera de servicio</option><option>Pendiente de repuesto</option></select></L></div><textarea value={technical.measurementsBefore} onChange={(e) => setTechnicalField("measurementsBefore", e.target.value)} rows={2} placeholder="Mediciones y parámetros antes de intervenir" className="u-input mt-2 resize-none" /><textarea value={technical.measurementsAfter} onChange={(e) => setTechnicalField("measurementsAfter", e.target.value)} rows={2} placeholder="Mediciones y parámetros finales" className="u-input mt-2 resize-none" /><textarea value={technical.testsPerformed} onChange={(e) => setTechnicalField("testsPerformed", e.target.value)} rows={2} placeholder="Pruebas funcionales realizadas" className="u-input mt-2 resize-none" /><textarea value={technical.testResult} onChange={(e) => setTechnicalField("testResult", e.target.value)} rows={2} placeholder="Resultados y criterios de aceptación" className="u-input mt-2 resize-none" /></Section>
        <Section title="Recomendaciones y pendientes"><textarea value={technical.recommendations} onChange={(e) => setTechnicalField("recommendations", e.target.value)} rows={3} placeholder="Recomendaciones técnicas para el cliente" className="u-input resize-none" /><textarea value={technical.pendingActions} onChange={(e) => setTechnicalField("pendingActions", e.target.value)} rows={2} placeholder="Acciones pendientes, responsable y prioridad" className="u-input mt-2 resize-none" /><L label="Fecha sugerida de seguimiento"><input type="date" value={technical.followUpDate} onChange={(e) => setTechnicalField("followUpDate", e.target.value)} className="u-input" /></L></Section>
        {ger && <Section title="Información interna"><div className="grid grid-cols-2 gap-2"><L label="Garantía"><input value={technical.warranty} onChange={(e) => setTechnicalField("warranty", e.target.value)} placeholder="Cobertura / vencimiento" className="u-input" /></L><L label="Recurrencia"><input value={technical.recurrence} onChange={(e) => setTechnicalField("recurrence", e.target.value)} placeholder="Primera vez / recurrente" className="u-input" /></L></div><textarea value={technical.internalNotes} onChange={(e) => setTechnicalField("internalNotes", e.target.value)} rows={3} placeholder="Notas privadas, riesgos comerciales o próximos pasos internos" className="u-input mt-2 resize-none" /></Section>}
        <Section title="Conformidad del cliente"><SignaturePad onChange={setSignatureUrl} /><input value={signedBy} onChange={(e) => setSignedBy(e.target.value)} placeholder="Nombre de quien firma" className="u-input mt-2" /><div className="mt-2 grid grid-cols-2 gap-2"><input value={technical.signerRole} onChange={(e) => setTechnicalField("signerRole", e.target.value)} placeholder="Cargo" className="u-input" /><input value={technical.signerCompany} onChange={(e) => setTechnicalField("signerCompany", e.target.value)} placeholder="Empresa" className="u-input" /></div>{!signatureUrl && (<div className="mt-2"><p className="mb-1 text-[11px] text-amber-600">Se recomienda la firma del cliente. Si no es posible, indica el motivo para poder completar igual:</p><input value={noSignReason} onChange={(e) => setNoSignReason(e.target.value)} placeholder="Motivo (ej. cliente ausente)" className="u-input" /></div>)}</Section>
        <Box className="p-4"><h3 className="mb-3 text-sm font-semibold text-slate-900">Resumen antes de enviar</h3><div className="space-y-1.5 text-sm text-slate-600"><div className="flex justify-between gap-3"><span>Cliente</span><span className="text-right font-medium text-slate-800">{client?.name || "—"}</span></div><div className="flex justify-between gap-3"><span>Servicio</span><span className="text-right font-medium text-slate-800">{service}</span></div><div className="flex justify-between gap-3"><span>Horas en planta</span><span className="font-medium text-slate-800">{laborHours || 0} h × {technicians || 1} técnico(s) = {(Number(laborHours) || 0) * (Number(technicians) || 1)} h-técnico</span></div><div className="flex justify-between gap-3"><span>Materiales</span><span className="font-medium text-slate-800">{materials.length}</span></div>{ger && <><div className="flex justify-between gap-3 border-t border-slate-100 pt-2"><span>Mano de obra</span><span>{money(preview.labor)}</span></div><div className="flex justify-between gap-3"><span>Materiales</span><span>{money(preview.mats)}</span></div><div className="flex justify-between gap-3 font-semibold text-slate-900"><span>Total</span><span>{money(preview.total)}</span></div></>}</div></Box>
        </>)}
      </main>
